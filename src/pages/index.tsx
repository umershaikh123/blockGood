...
const Home: NextPage = () => {
  ...
  const createAttestation = async () => {
    ...
    try {
      ...
      alert(`Donation attestation created successfully! ID: ${attestationResult.attestationId}`);
      setCampaignId('');
      setAmount('');
          ...
          <ul>
            {attestations.map((attestation) => (
              <li key={attestation.id} className="mb-4 p-4 border rounded">
                ...
                <p><strong>Timestamp:</strong> {attestation.attestTimestamp}</p>
                {attestation.data.campaignId && <p><strong>Campaign ID:</strong> {attestation.data.campaignId}</p>}
                {attestation.data.donorAddress && <p><strong>Donor Address:</strong> {attestation.data.donorAddress}</p>}
                {attestation.data.Amount && (
                  <p><strong>Amount:</strong> {formatEther(BigInt(attestation.data.Amount))} ETH</p>
                )}
                {attestation.data.timeStamp && (
                  <p><strong>Donation Time:</strong> {new Date(attestation.data.timeStamp).toLocaleString()}</p>
                )}
              </li>
            ))}
          </ul>
